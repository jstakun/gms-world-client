...
public class DealCategoryListActivity extends ListActivity implements View.OnClickListener {
    ...
    protected void onClickAction(int position, String action) {
        if (action.equals("drill")) {
            ...
            if (!landmarkManager.isLayerEmpty(c)) {
                ...
                intents.showInfoToast(Locale.getMessage(com.jstakun.gms.android.ui.lib.R.string.Landmark_search_empty_result));
            }
        }
    }
    ...
    public void onResume() {
        super.onResume();
        if (ConfigurationManager.getInstance().containsObject(ConfigurationManager.SEARCH_QUERY_RESULT, String.class)) {
            if (StringUtils.equals((String) ConfigurationManager.getInstance().getObject(ConfigurationManager.SEARCH_QUERY_RESULT, String.class), "true")) {
                Intent result = new Intent();
                setResult(RESULT_OK, result);
                finish();
            } else if (StringUtils.equals((String) ConfigurationManager.getInstance().removeObject(ConfigurationManager.SEARCH_QUERY_RESULT, String.class), "false")) {
    ...
    private class CategoryComparator implements Comparator<Category> {
        ...
        private CategoryCountComparator categoryCountComparator =

...
public class CalendarView extends ImageView {
    ...
    private void initCells() {
        ...
        // build cells
        int[] landmarksInDays = new int[31];
        for (int i = 0; i < 31; i++) {
            landmarksInDays[i] = 0;
        }
        ...
        if (lm != null) {
            lm.findLandmarksInMonth(mHelper.getYear(), mHelper.getMonth(), landmarksInDays);
        }
        ...
        for (int week = 0; week < mCells.length; week++) {
            for (int day = 0; day < mCells[week].length; day++) {
                if (tmp[week][day].thisMonth) {
                    boolean hasLandmarks = (tmp[week][day].thisMonth && landmarksInDays[tmp[week][day].day - 1] == 1);
                    if (day == 0 || day == 6) {
                        mCells[week][day] = new RedCell(tmp[week][day].day, new Rect(Bound), CELL_TEXT_SIZE, hasLandmarks);
                    } else {
                        mCells[week][day] = new Cell(tmp[week][day].day, new Rect(Bound), CELL_TEXT_SIZE, hasLandmarks);
                    }
                } else {
                    mCells[week][day] = new GrayCell(tmp[week][day].day, new Rect(Bound), CELL_TEXT_SIZE, false);
                }
    ...
    protected class GrayCell extends Cell {

        public GrayCell(int dayOfMon, Rect rect, float s, boolean selected) {
            super(dayOfMon, rect, s, selected);
    ...
    private class RedCell extends Cell {
        ...
        public RedCell(int dayOfMon, Rect rect, float s, boolean selected) {
            super(dayOfMon, rect, s, selected);
